...
\definition Пусть $X = (X, \MM, \mu)$ - пространство с мерой, $Y$ - параметрическое множество, $f : X \times Y \ra \overline{\R}$ функция, такая что $\forall y \in Y$ она будет интегрируемой, то есть $f \in L_1(X)$, тогда введём функцию $J(y) := \int\limits_X f(x, y) d\mu(x) \in \overline{\R}$ - собственный интеграл Лебега, зависящий от параметра.
\\
...
\theorem Пусть дополнительно известно, что $Y = (Y, d)$ - метрическое пространство. $y_0$ - предельная точка в $Y$, а также пусть $\exists \widetilde{X} \subset X$, т.ч. $\mu(X \bs \widetilde{X}) = 0$, а также:
\begin{enumerate}
...
\definition Будем говорить, что $\int\limits^{\ra b}_{a} f(t, y) dt$ сходится равномерно по $y \in Y$, если $\forall y \in Y$ он сходится и при этом $\sup \left| \int\limits^{\ra b}_{b'} f(t, y) dt \right| \ra 0, \; b' \ra b - 0$
